...
let
  ...
  defaultCabalPackageVersionComingWithGhc = { ghc884 = "Cabal_3_2_1_0"; }."${compiler}";

  static-haskell-nix = fetchTarball sources.static-haskell-nix.url;

  survey = import "${static-haskell-nix}/survey" { 
    inherit compiler defaultCabalPackageVersionComingWithGhc;
    normalPkgs = nixpkgs; 
  };
in
  { inherit compiler nixpkgs easy-purescript-nix survey;
  }
