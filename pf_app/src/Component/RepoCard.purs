...
render repositories =
  HH.div
  [ css "h-full flex flex-wrap justify-center" ]
  cards
  where
    makeCard :: forall w i. Repository -> HH.HTML w i
    makeCard repository =
      HH.a
      [ css "md:w-1/3 w-full h-40 flex flex-col flex-none bg-white rounded-xl divide-solid divide-y-2 md:shadow-lg shadow-md m-2 transform hover:-translate-y-2 cursor-pointer"
